...
# Variables
lf_bg = 'Grey15'  # Left Frame Background Color
rtf_bg = 'LightGoldenrod1'  # Right Top Frame Background Color
rbf_bg = 'DodgerBlue'  # Right Bottom Frame Background Color
btn_hlb_bg = 'Gold2'  # Background color for Head Labels and Buttons
...
root = Tk()
root.title('Library Management System by Varun')
...
RT_frame.place(x=0, y=30, relheight=0.2, relwidth=0.7)

RB_frame = Frame(root)
RB_frame.place(x=0, rely=0.24, relheight=0.785, relwidth=0.7)
...
# Right Top Frame
Label(RT_frame, text='Made by Varun Mehra', font=("Arial", 20, 'italic'), bg=rtf_bg, fg='Black').place(x=275, y=10)
Button(RT_frame, text='Delete book record', font=btn_font, bg=btn_hlb_bg, width=17, command=remove_record).place(x=8, y=40)
Button(RT_frame, text='Delete full inventory', font=btn_font, bg=btn_hlb_bg, width=17, command=delete_inventory).place(x=178, y=40)
Button(RT_frame, text='Update book details', font=btn_font, bg=btn_hlb_bg, width=17,
       command=update_record).place(x=348, y=40)
Button(RT_frame, text='Change Book Availability', font=btn_font, bg=btn_hlb_bg, width=19,
       command=change_availability).place(x=518, y=40)

# Right Bottom Frame
Label(RB_frame, text='BOOK INVENTORY', bg='LightYellow2', fg='Black', font=("Noto Sans CJK TC", 15, 'bold')).pack(side=TOP, fill=X)
